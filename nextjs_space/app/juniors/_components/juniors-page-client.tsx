      ...
      <section className="relative bg-brand-purple overflow-hidden">
        ...
        <div className="relative max-w-7xl mx-auto px-4 py-16 md:py-24 text-center">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
            <span className="inline-block px-4 py-1.5 bg-brand-neon/20 text-brand-neon text-xs font-bold uppercase tracking-widest rounded-full mb-4">⭐ Next Gen</span>
            <h1 className="text-4xl md:text-6xl font-heading font-black text-white mb-4">{t('juniors.title')}</h1>
            <p className="text-lg md:text-xl text-white/70 max-w-2xl mx-auto">{t('juniors.subtitle')}</p>
          </motion.div>
        </div>
      </section>
                ...
                <Link href={getArticlePath(junior?.slug ?? '', junior?.category ?? 'juniors')} className="block">
                  <div className="relative aspect-[3/4] rounded-xl overflow-hidden bg-gray-100 mb-4">
                    {junior?.imageUrl && (
                      <Image
                        src={junior.imageUrl}
                        alt={junior?.title ?? 'Junior player'}
                        fill
                    ...
                    <div className="absolute bottom-0 left-0 right-0 p-4">
                      <h3 className="text-lg font-heading font-bold text-white mb-1">{junior?.authorName ?? ''}</h3>
                    </div>
                  </div>
