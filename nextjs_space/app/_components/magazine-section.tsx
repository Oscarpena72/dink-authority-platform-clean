                ...
                <div className="grid grid-cols-1 md:grid-cols-2">
                  <div className="relative aspect-[3/4] bg-brand-purple-light">
                    {currentEdition?.coverUrl ? (
                      ...
                      <div className="absolute inset-0 flex items-center justify-center blue-gradient">
                        <BookOpen size={64} className="text-white/30" />
                      </div>
                    )}
                    <div className="absolute top-4 left-4">
                      <span className="px-3 py-1.5 bg-brand-accent text-white text-[11px] font-bold uppercase tracking-widest rounded shadow-lg">Current Issue</span>
                    </div>
                  </div>
                  <div className="p-8 flex flex-col justify-center">
                    {currentEdition?.issueNumber && (
                      <span className="text-brand-blue text-xs font-bold uppercase tracking-widest mb-2">{currentEdition.issueNumber}</span>
                    )}
                    ...
                    {currentEdition?.externalUrl && (
                      <a
                        ...
                        className="inline-flex items-center gap-2 px-5 py-2.5 bg-brand-blue text-white font-bold rounded-lg hover:bg-brand-blue-light transition-all text-sm uppercase tracking-wider self-start"
                      >
          ...
          {pastEditions.length > 0 && (
            <div className="lg:col-span-2 flex flex-col gap-4">
              ...
              {pastEditions.map((edition: EditionItem, i: number) => (
                ...
                >
                  <div className="bg-white rounded-xl overflow-hidden border border-gray-100 hover:border-brand-blue/20 transition-all group sport-card">
                    <div className="flex items-center gap-4 p-4">
                      ...
                      <div className="flex-1 min-w-0">
                        {edition?.issueNumber && (
                          <span className="text-brand-blue text-[10px] font-bold uppercase tracking-widest">{edition.issueNumber}</span>
                        )}
                        <h4 className="font-heading font-bold text-sm text-brand-purple group-hover:text-brand-blue transition-colors line-clamp-2">{edition?.title ?? ''}</h4>
                      ...
                      {edition?.externalUrl && (
                        <a href={edition.externalUrl} target="_blank" rel="noopener noreferrer" className="text-brand-blue hover:text-brand-blue-dark flex-shrink-0">
