...
export default async function HomePage() {
  ...
  try {
    heroArticle = await prisma.article.findFirst({
      where: { isHeroStory: true, status: 'published', locale: 'en' },
  ...
  try {
    latestArticles = await prisma.article.findMany({
      where: { status: 'published', locale: 'en', isHeroStory: false },
  ...
  try {
    editions = await prisma.magazineEdition.findMany({
      ...
      take: 4,
      select: { id: true, title: true, slug: true, issueNumber: true, coverUrl: true, description: true, pdfCloudPath: true, externalUrl: true, isCurrent: true, currentFor: true, publishDate: true },
    });
  } catch { /* empty */ }
