...
const CATEGORIES = ['technique', 'strategy', 'fitness', 'mental-game', 'equipment'];

interface TipFormProps {
  tip?: any;
}

export default function TipFormClient({ tip }: TipFormProps) {
  ...
  useEffect(() => {
    try {
      const parsed = JSON.parse(tip?.galleryImages ?? '[]');
      ...
      <form onSubmit={handleSubmit} className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="bg-white rounded-xl p-6 shadow-sm">
            ...
            <input name="title" value={form.title} onChange={handleChange} required className="w-full px-4 py-3 rounded-lg border border-gray-200 focus:border-brand-purple focus:ring-1 focus:ring-brand-purple outline-none text-lg font-heading" placeholder="Tip title..." />
          </div>
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input name="videoCtaText" value={form.videoCtaText} onChange={handleChange} className="w-full px-3 py-2 rounded-lg border border-gray-200 focus:border-brand-purple outline-none text-sm" placeholder="Watch full video" />
                </div>
                <div>
                  <label className="block text-xs text-gray-500 mb-1">CTA Link</label>
                  <input name="videoCtaLink" value={form.videoCtaLink} onChange={handleChange} className="w-full px-3 py-2 rounded-lg border border-gray-200 focus:border-brand-purple outline-none text-sm" placeholder="https://..." />
                </div>
              </div>
            </div>
          </div>
            ...
            <div className="space-y-2">
              {gallery.map((img, i) => (
                <div key={i} className="flex gap-2">
                  ...
                  <button type="button" onClick={() => removeGalleryImage(i)} className="p-2 text-red-400 hover:text-red-600">
                    <Trash2 size={16} />
                  </button>
                </div>
              ))}
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white rounded-xl p-6 shadow-sm">
            <h3 className="text-sm font-semibold text-brand-purple mb-4">Details</h3>
            <div className="space-y-3">
              <div>
              ...
              <div>
                ...
                <select name="category" value={form.category} onChange={handleChange} className="w-full px-3 py-2 rounded-lg border border-gray-200 focus:border-brand-purple outline-none text-sm">
                  {CATEGORIES.map((c) => (
              ...
              <div>
                ...
                <input name="featuredImage" value={form.featuredImage} onChange={handleChange} className="w-full px-3 py-2 rounded-lg border border-gray-200 focus:border-brand-purple outline-none text-sm" placeholder="https://www.elegantthemes.com/blog/wp-content/uploads/2017/04/divi-images-featured.jpg" />
              </div>
              {form.featuredImage && (
          ...
          <button type="submit" disabled={saving} className="w-full flex items-center justify-center gap-2 px-6 py-3 bg-brand-purple text-white rounded-xl hover:bg-brand-purple-light transition-colors font-semibold disabled:opacity-50">
            ...
            {saving ? 'Saving...' : (isEdit ? 'Update Tip' : 'Create Tip')}
          </button>
        </div>
      </form>
    </div>
