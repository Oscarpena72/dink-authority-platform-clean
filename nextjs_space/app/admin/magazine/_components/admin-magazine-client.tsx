...
import { BookOpen, Plus, Pencil, Trash2, Save, X, Star } from 'lucide-react';
...
const EMPTY_FORM = {
  title: '', issueNumber: '', coverUrl: '', description: '', externalUrl: '', isCurrent: false, publishDate: '',
};
      ...
      {showForm && (
        <div className="bg-white rounded-xl border border-gray-200 p-6 mb-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <input placeholder="Title" value={form.title} onChange={e => setForm(p => ({ ...p, title: e?.target?.value ?? '' }))} className="px-3 py-2 border border-gray-200 rounded-lg text-sm focus:border-brand-blue focus:outline-none" />
            <input placeholder="Issue Number (e.g. Issue #12)" value={form.issueNumber} onChange={e => setForm(p => ({ ...p, issueNumber: e?.target?.value ?? '' }))} className="px-3 py-2 border border-gray-200 rounded-lg text-sm focus:border-brand-blue focus:outline-none" />
            <input placeholder="Cover Image URL" value={form.coverUrl} onChange={e => setForm(p => ({ ...p, coverUrl: e?.target?.value ?? '' }))} className="px-3 py-2 border border-gray-200 rounded-lg text-sm focus:border-brand-blue focus:outline-none" />
            <input placeholder="External URL (link to read)" value={form.externalUrl} onChange={e => setForm(p => ({ ...p, externalUrl: e?.target?.value ?? '' }))} className="px-3 py-2 border border-gray-200 rounded-lg text-sm focus:border-brand-blue focus:outline-none" />
            <input type="date" value={form.publishDate} onChange={e => setForm(p => ({ ...p, publishDate: e?.target?.value ?? '' }))} className="px-3 py-2 border border-gray-200 rounded-lg text-sm focus:border-brand-blue focus:outline-none" />
            <label className="flex items-center gap-2 px-3 py-2">
              <input type="checkbox" checked={form.isCurrent} onChange={e => setForm(p => ({ ...p, isCurrent: e?.target?.checked ?? false }))} className="w-4 h-4 rounded border-gray-300 text-brand-blue focus:ring-brand-blue" />
              <span className="text-sm font-medium">Mark as Current Issue</span>
            </label>
          </div>
          <div className="mt-4">
            <textarea placeholder="Description" value={form.description} onChange={e => setForm(p => ({ ...p, description: e?.target?.value ?? '' }))} rows={3} className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:border-brand-blue focus:outline-none" />
          </div>
          ...
          (editions ?? []).map((ed: EditionItem) => (
            <div key={ed?.id} className="bg-white rounded-xl border border-gray-200 p-5 hover:shadow-md transition-shadow">
              <div className="flex items-start justify-between mb-3">
                <div>
                  {ed?.issueNumber && <span className="text-brand-blue text-xs font-bold uppercase tracking-wider">{ed.issueNumber}</span>}
              ...
              <div className="flex items-center gap-2">
                <button onClick={() => handleEdit(ed)} className="p-1.5 text-brand-blue hover:bg-brand-blue/10 rounded"><Pencil size={14} /></button>
